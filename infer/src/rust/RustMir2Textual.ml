...
let mk_decl crate (fun_decl : Charon.UllbcAst.blocks Charon.GAst.gfun_decl) :
  ...
  (* Filter UNIT_METADATA for now so that we can avoid duplicate functions in pulse test *)
  if String.( = ) (item_meta_to_string crate fun_decl.item_meta) "UNIT_METADATA" then None
  else
    try
      match fun_decl.body with
      | Some body -> (
...
let mk_global crate (global_decl : Charon.Generated_GAst.global_decl) =
  (* Filter UNIT_METADATA for now so that we can avoid duplicate functions in pulse test *)
  if String.( = ) (item_meta_to_string crate global_decl.item_meta) "UNIT_METADATA" then None
  else
...
let mk_module (crate : Charon.UllbcAst.crate) ~file_name : Textual.Module.t =
  ...
  *)
  ...
  let global_vars =
    ...
    |> List.filter_map ~f:(mk_global crate)
  in
